...
import css from './ScrollReveal.module.css?raw'

const SOURCE = [
  '// 使い方：出したい要素を包むだけ',
  '<ScrollReveal>{card}</ScrollReveal>',
  '<ScrollReveal settle="smooth">{card}</ScrollReveal>       // 吸い付いて止まる',
...
const TIMELINE = [
  '### 時系列（要素が amount ぶん見えた瞬間 = 0 秒）',
  '',
  'IntersectionObserver が「amount（既定 0.3）だけ見えた」を検知して .visible を付与。',
  'あとは CSS transition が opacity と translateY を duration かけて動かす。',
  '',
  '- **reveal: 現れる要素**（下から distance px・opacity 0 → 定位置・opacity 1）',
  ...
  '',
  '| 時刻 | reveal: transform | reveal: opacity |',
  '|---|---|---|',
  '| 発火前 | translateY(+distance) | 0 |',
  '| 0.00s | .visible 付与、transition 開始 | 0 |',
  '| ~0.6s | わずかに 0 を通り越す（snap の overshoot） | ~1 |',
  '| ~0.8s（duration） | translateY(0) に着地 | 1 |',
].join('\n')
...
const meta = {
  title: 'Animations/Scroll Reveal/CSS',
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'motion を使わず、IntersectionObserver（見えた判定）+ CSS transition（動き）だけで作る' +
          'スクロール出現。距離・速度は props（CSS 変数経由）、once で「1 回だけ / 毎回」、' +
          'settle で着地の質感（snap / smooth）を切り替える。複数並べると順に上がってくる。\n\n' +
          TIMELINE,
      },
    },
  },
...
const SECTIONS = [
  ...
  { id: 2, title: 'Section 2', desc: '各要素が「自分が画面に入った瞬間」に発火します。' },
  { id: 3, title: 'Section 3', desc: '判定は IntersectionObserver、動きは CSS transition。' },
  { id: 4, title: 'Section 4', desc: 'settle="snap" なので定位置を少し通り越して収まります。' },
  { id: 5, title: 'Section 5', desc: 'motion 版と同じ見た目を、標準機能だけで実現しています。' },
]
